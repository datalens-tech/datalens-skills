...
- **Write skills in English.** A skill's `description` is the text the agent reads to decide when
  to fire it, and model tool-selection and instruction-following are most reliable in English — so
  keep the `description` and the `SKILL.md` instructions in English unless there's a clear reason
  not to. Localize what a skill *produces*, not the skill itself: `datalens-html-pages` is written
  in English yet teaches the generated page to honor the user's `ru`/`en` DataLens language.
- One folder per skill under `skills/`, each with a valid `SKILL.md`
  (check with `node scripts/validate_skills.mjs`).
